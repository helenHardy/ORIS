...
-- General Policy: Read for authenticated
DO $$ 
DECLARE 
    t text;
BEGIN
    FOR t IN (SELECT tablename FROM pg_tables WHERE schemaname = 'public') LOOP
        IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = t AND policyname = 'Permitir lectura para autenticados') THEN
            EXECUTE format('CREATE POLICY "Permitir lectura para autenticados" ON public.%I FOR SELECT USING (auth.role() = ''authenticated'');', t);
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = t AND policyname = 'Permitir gestión para Administradores') THEN
             EXECUTE format('CREATE POLICY "Permitir gestión para Administradores" ON public.%I FOR ALL USING (EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = ''Administrador''));', t);
        END IF;
    END LOOP;
END $$;
...
-- Example for POS:
CREATE POLICY "Gestión operativa para autenticados sales" ON public.sales FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Gestión operativa ítems para autenticados sale_items" ON public.sale_items FOR ALL USING (auth.role() = 'authenticated');
-- (Extend as needed)
...
CREATE OR REPLACE TRIGGER trg_sync_global_stock
